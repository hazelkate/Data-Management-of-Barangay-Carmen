        ...
        class="container">
          ...
          <template v-slot:expanded-item="{ headers, item }">
            <td :colspan="headers.length">
              ...
              <v-subheader>Incident Involve</v-subheader>
              <v-simple-table class="my-2">
                ...
                <tbody>
                    <tr
                     v-for="(innerItem,key) in item.attributes.involve"
                     :key="key"
                    >
                    <td>{{innerItem.attributes.firstname}}</td>
                    <td>{{innerItem.attributes.middlename}}</td>
                    <td>{{innerItem.attributes.lastname}}</td>
                    </tr>
                </tbody>
                </template>
            </v-simple-table>
  ...
  export default {
    ...
    methods: {
      ...
      async createData (){
        ...
        let involve = this.editedItem.involve
        let respondent = Moralis.User.current()

        const Incident = Moralis.Object.extend("Incident");
        const incident = new Incident();
          ...
          await incident.save({
            ...
            .then((success) => {
              ...
              notification.set("content", "Added a Incident data");
      ...
      async init (){
        const response = await Moralis.Cloud.run("getIncident");
        this.incident =  response.reverse();
      },


      async initSocket() {
        let query = new Moralis.Query('Incident');
        let subscription = await query.subscribe();


        subscription.on('create', (object) => {
          this.incident.unshift(object);
          this.$store.dispatch('snackbar/setSnackbar', {
        ...
        subscription.on('delete', (object) => {
          ...
          this.incident.splice(this.editedIndex, 1)
          this.closeDelete()
        });
      ...
      editItem (item) {
        this.activeObj = item;
        this.editedIndex = this.incident.indexOf(item)
