  ...
  export default {
    data: () => ({
      ...
      headers: [
        ...
        { text: 'respondent',  value: 'attributes.respondent.attributes.lastname'},
    ...
    methods: {
      ...
      async init (){
        const response = await Moralis.Cloud.run("getLupon");
        this.blotter =  response.reverse();
      },


      async initSocket() {
        let query = new Moralis.Query('Lupon');
        let subscription = await query.subscribe();


        subscription.on('create', (object) => {
          this.blotter.unshift(object);
          this.$store.dispatch('snackbar/setSnackbar', {
        ...
        subscription.on('delete', (object) => {
          ...
          this.blotter.splice(this.editedIndex, 1)
          this.closeDelete()
        });
      ...
      editItem (item) {
        this.activeObj = item;
        this.editedIndex = this.blotter.indexOf(item)
